...
class _EditCategoryDialog extends State<EditCategoryDialog> {
  ...
  void initState() {
    ...
    if (categoryName == "标签")
      _editTextController =
          TextEditingController(text: RuntimeData.labelList[_index]);
    else
      _editTextController =
          TextEditingController(text: RuntimeData.folderList[_index]);
  }
      ...
      if (categoryName == "标签") {
        if (!RuntimeData.labelList.contains(_editTextController.text)) {
          ...
          Fluttertoast.showToast(
              msg: "保存$categoryName ${_editTextController.text}");
          Navigator.pop<bool>(context, true);
        } else {
          Fluttertoast.showToast(
              msg: "$categoryName ${_editTextController.text} 已存在");
          Navigator.pop<bool>(context, false);
        }
      } else {
  ...
  editLabelAndUpdate() async {
    List<PasswordBean> passwordList =
        Provider.of<PasswordList>(context).passwordList;
