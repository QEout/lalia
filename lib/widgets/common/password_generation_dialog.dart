...
class _PasswordGenerationDialog extends State<StatefulWidget> {
  ...
  List<String> _numberList = List()
    ..addAll(['1', '9', '0', '2', '5', '6', '3', '4', '7', '8']);

  List<String> _symbolList = List()
    ..addAll(['!', '@', '*', '?', '#', '%', '~', '=']);

  @override
  void initState() {
    _controller.text = generation(_capitalChoose, _lowerCaseChoose,
        _numberChoose, _symbolChoose, _length.floor());
    super.initState();
  }
  ...
  String generation(bool cap, bool low, bool number, bool sym, int len) {
    ...
    Random random =
        Random(DateTime.now().hashCode - (list.length + len).hashCode);
    for (int i = 0; i < len; i++) {
