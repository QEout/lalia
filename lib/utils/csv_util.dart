...
class CsvUtil {
  ...
  Future<String> passwordExportCsv(
      List<PasswordBean> list, Directory dst) async {
    if (Platform.isAndroid) {
      String path = "${dst.path}/lalia_密码.csv";
      File csv = File(path);
      if (!csv.existsSync()) {
        csv.createSync();
      }
  ...
  Future<String> cardExportCsv(List<CardBean> list, Directory dst) async {
    if (Platform.isAndroid) {
      String path = "${dst.path}/lalia_卡片.csv";
      File csv = File(path);
      if (!csv.existsSync()) {
        csv.createSync();
      }
  ...
  Future<List<PasswordBean>> passwordImportFromCsv(
      {String path, String toParseText}) async {
    ...
    if (text.length <= 1)
      ...
      for (var item in text.sublist(1)) {
        ...
        if (indexMap.containsKey('label') &&
            attribute[indexMap['label']].length > 0) {
          label = waveLineSegStr2List(
              getValueWithCatch(attribute, indexMap, 'label'));
        }
  ...
  String getValueWithCatch(
      List<String> values, Map<String, int> indexMap, String mapKey) {
    try {
